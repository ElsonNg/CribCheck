...
abstract class DBService<T> {
     ...
     * @returns {Promise<T | null>} A promise that resolves to the user profile data if found, or 'null' if no profile exists.
     */
    abstract loadUserProfile(userId: string): Promise<T | null>;
    /**
     ...
     * @returns {Promise<T | null>} a promise that resolves when the operation is complete
     */
    abstract saveUserProfile(userProfileData: T, userId: string): Promise<T | null>;
